package com.datametica.cockatoos.output.writers.instrumentation
...
import com.datametica.cockatoos.output.MetricOutputWriter
    ...
    dataFrame.foreach(row => {
      for ((column, i) <- columns) {
        try {
          ...
          case ex: Throwable => {
            throw CockatoosWriteFailedException(s"failed to write instrumentation on data frame: $dataFrameName " +
